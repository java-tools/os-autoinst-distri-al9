...
sub run {
        ...
        my $id = get_var("DISTRI"); # Should be "almalinux"

        my $name = ucfirst($id);
    # from F35 onwards, $NAME is "Fedora Linux" not just "Fedora"
    #    my $relnum = get_release_number;
    #    my $fullname = $relnum > 34 ? $name . " Linux" : $name;        # $NAME is "AlmaLinux" not just "AlmaLinux"
        my $fullname = $name;
        ...
        # Test for AlmaLinux Support Product
        $strip = strip_marks($content{'ALMALINUX_SUPPORT_PRODUCT'});
        if ( $ver_major = '9' ) {
            $fullname = qq/$fullname $ver_major/;
            $fullname =~ s/ /-/g;
        }
        rec_log "ALMALINUX_SUPPORT_PRODUCT should be $fullname and is $strip", $strip eq $fullname, $failref;

        # Test for AlmaLinux Support Product Version
        $strip = strip_marks($content{ALMALINUX_SUPPORT_PRODUCT_VERSION});
        rec_log "ALMALINUX_SUPPORT_PRODUCT_VERSION should be $target and is $strip", $strip eq $target, $failref;
